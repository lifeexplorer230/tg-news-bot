...
class TestChannelOperations:
    ...
    def test_add_duplicate_channel(self, temp_db):
        ...
        channel_id1 = temp_db.add_channel('test_channel', 'Test Channel')
        channel_id2 = temp_db.add_channel('test_channel', 'Test Channel 2')

        # При дубликате может вернуться None или тот же ID
        # В текущей реализации возвращается None, проверим что не создался новый
        cursor = temp_db.conn.cursor()
...
class TestMessageOperations:
    ...
    def test_mark_as_processed_with_rejection(self, temp_db):
        ...
        temp_db.mark_as_processed(
            msg_id,
            rejection_reason='rejected_by_keywords'
        )
