...
def run_scheduler():
    """
    Запустить планировщик задач с adaptive idle time

    Использует schedule.idle_seconds() для оптимального времени ожидания
    вместо жёсткого sleep(60), что позволяет:
    - Быстрее реагировать на запланированные задачи
    - Поддерживать интервалы < 60 секунд
    - Избежать дрожания таймингов
    """
    global running
    ...
    while running:
        ...
        # Adaptive idle time: сколько секунд до следующей задачи
        idle = schedule.idle_seconds()

        # Edge case: пустое расписание (idle = None)
        if idle is None:
            # Нет задач в расписании - используем default sleep
            sleep_time = 60
        elif idle < 0:
            # Пропущенные задачи - сразу следующая итерация
            sleep_time = 0
        else:
            # Safety sleep: max 5 секунд для предотвращения busy-wait
