...
- **Профили конфигурации** - быстрое переключение между разными наборами настроек (marketplace, ai, generic, reels)
...
- **🎬 Генерация сценариев Reels** - автоматическое создание сценариев для Instagram Reels через Perplexity API (НОВОЕ!)
...
- Perplexity API key (для модуля Reels Generator - опционально)
...
│   │   ├── reels.yaml       # Профиль для генерации Reels сценариев
...
│
├── reels/                  # 🎬 Модуль генерации Reels сценариев
│   ├── models/             # Модели данных (News, ReelsScenario)
│   ├── services/           # Perplexity Client, Reels Processor
│   ├── config/             # Конфигурация модуля
│   ├── prompts/            # Промпты для Perplexity AI
...
### 🎬 Reels Generator (Генератор сценариев)
...
**Что делает Reels Generator:**

1. Получает новости из БД (таблица published)
2. Обогащает через Perplexity API (дополнительный контекст, факты, предыстория)
3. Генерирует структурированный сценарий:
